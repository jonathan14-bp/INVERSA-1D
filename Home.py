...
with col_dataset:
    st.markdown(
        '<p class="section-header">Input Dataset</p>',
        unsafe_allow_html=True
    )
        ...
        if st.button(
            ...
            try:
                ...
                if np.any(r_synthetic <= 0):
                    ...
                    # Adaptive AB/2 sampling so H, K, A, and Q curve shapes
                    # can be displayed more fully.
...
# =============================================================================
# COLUMN 2: INVERSION PARAMETERS
# =============================================================================
with col_method:
    st.markdown(
        '<p class="section-header">Inversion Parameters</p>',
        unsafe_allow_html=True
    )
    ...
    if st.session_state.data_loaded:
        ...
        noise_locked = current_data_mode in (MODE_MANUAL, MODE_UPLOAD)

        noise_test_help = (
            "Noise Test adds artificial Gaussian noise to the observed "
            "apparent resistivity data to simulate measurement uncertainty. "
            "This is only meaningful for synthetic data generated from a "
            "known true model, since field VES data already contain their "
            "own real-world noise."
        ) if noise_locked else None

        if noise_test_help:
            noise_info_icon = (
                f'<span title="{escape(noise_test_help)}" '
                'style="cursor:help; color:#94a3b8; font-weight:700; '
                'margin-left:0.35rem;">&#9432;</span>'
            )
        else:
            noise_info_icon = ""

        st.markdown(
            f'<p class="sublabel">Noise Test{noise_info_icon}</p>',
            unsafe_allow_html=True
        )
        ...
        n_layers_input = st.number_input(
            ...
            step=1,
            label_visibility="collapsed",
            help="Number of layers for the inversion model. Can be changed "
                 "freely, including while in New Synthetic Data mode."
        )
        ...
        with tab_inversion:
            ...
            with row_plots[1]:
                ...
                if reference_rho_result is not None:
                    all_r = np.concatenate([all_r, [reference_rho_result]])
                ...
                if reference_rho_result is not None:
                    reference_label = f"Initial Model ({reference_rho_result:g} Ohm.m)"

                    ax2.axvline(
                        x=reference_rho_result,
                        ymin=0.0,
                        ymax=1.0,
                        color="gray",
                        linestyle="--",
